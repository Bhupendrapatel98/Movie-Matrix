...
class TrendingMoviePagingSource(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Result>): MediatorResult {
        ...
        try {
            val apiResponse = repository.getTrendingMovie(page = page)
            ...
            moviesDatabase.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    ...
                    moviesDatabase.getMoviesDao().clearMoviesByType("movie")
                }
            ...
            Log.d("IOException", "load: "+error.message)
            return MediatorResult.Error(error)
        } catch (error: HttpException) {
            Log.d("HttpException", "load: "+error.message)
            return MediatorResult.Error(error)
        }
    }
