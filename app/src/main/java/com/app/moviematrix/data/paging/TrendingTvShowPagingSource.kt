...
class TrendingTvShowPagingSource(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Result>): MediatorResult {
        ...
        try {
            val apiResponse = repository.getTrendingTvShow(page)
            ...
            moviesDatabase.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    ...
                    moviesDatabase.getMoviesDao().clearMoviesByType("tvshow")
                }
            ...
            Log.d("jdsh748743", "load: " + error.message)
            return MediatorResult.Error(error)
        } catch (error: IOException) {
            Log.d("jdsh7487434", "load: " + error.message)
            return MediatorResult.Error(error)
        }
    }
